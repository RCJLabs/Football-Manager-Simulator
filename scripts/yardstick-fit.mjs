...
for (const p of men) {
  ...
  let pf = 0;
  for (let i = 0; i < N; i++) { const g = createGame(tf(t), tf(OPP), { seed: 31000 + i, homeAdvantage: false }); simulateGame(g); pf += g.score[0]; }
  rows.push({ p, pf: pf / N, ovr: overall(p) });
}
const corr = (xs, ys) => {
...
console.log(`  overall vs points produced:  r = ${corr(rows.map((r) => r.ovr), pf).toFixed(3)}`);
console.log('\n  each attribute on its own:');
const byR = attrs.map((a) => ({ a, r: corr(rows.map((x) => x.p.r[a] ?? 0), pf) })).sort((x, y) => Math.abs(y.r) - Math.abs(x.r));
for (const { a, r } of byR) console.log(`    ${a.padEnd(5)} r = ${r.toFixed(3)}`);
console.log(`\n  rating range covered: ${Math.min(...rows.map((r) => r.ovr))} to ${Math.max(...rows.map((r) => r.ovr))}`);
console.log(`  points range:         ${Math.min(...pf).toFixed(1)} to ${Math.max(...pf).toFixed(1)}`);
