...
const CHECKS = [
  ...
  },
  {
    ...
    doc: /the backs'\s+slope\s+\(([\d.]+),\s+flagged/,
  ...
  },
  {
    ...
    why: 'the 58 quarterbacks in the pool with a real season from 1999 on, each in the same average side, against what each really did (nflverse): the slope of real adjusted net yards a dropback on the engine\'s. 1 would be the engine\'s differences between quarterbacks coming true one for one. This is pinned where the engine is, not where it should be: it read 0.41 when first measured on 2026-09-24 and the passing game was left as it was, so a change to the passer\'s terms trips it on purpose, and DESIGN.md ("The passing game, against real quarterbacks") has to be rewritten with it. About thirty seconds, shared with the check below',
  },
  {
    ...
    expect: 7.18, tol: 0.3,
    why: 'drafted pro leagues throw for 7.18 adjusted net yards a dropback against a real 5.84 to 6.18 (2019-2023), because several passing terms are measured against a fixed number and a league of greats sits far above it. Pinned where the engine is, not where the real game is: the passing game was measured and left as it was on 2026-09-24, so this trips when anything moves it, the intended fix included',
  },
  {
