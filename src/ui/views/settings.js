...
export function view(root, params, ctx) {
    ...
    <div class="grid grid-2">
      <div class="card">
        ...
        ${league ? html`
          ...
          ${league.mode === 'pro' ? html`<label class="check" style="margin-top:.4rem"><input type="checkbox" id="weather" ${league.settings.weather ? 'checked' : ''}> Weather — each club's city brings its own: wind and cold cost passing and kicking, rain and snow cost ball security, and a covered stadium plays indoors</label>` : ''}
          ${league.mode === 'pro' ? html`<label class="check" style="margin-top:.4rem"><input type="checkbox" id="positions" ${league.settings.positions ? 'checked' : ''}> Position changes — move a man to a position his skills translate to, safety to corner and the like; a move up the market's scale is paid for</label>` : ''}
          ...
          ${league.mode === 'pro' ? '' : html`<small class="muted" id="proOnly" style="display:block;margin:.4rem 0 .2rem">Weather, position changes and coaching jobs are pro-league settings, and a pro league also has a salary cap, contracts, free agency and a practice squad. A league cannot change kind: start a pro league from the home screen to play one. It takes its own save slot.</small>`}
          ${league.settings.careers && league.settings.chemistry && league.settings.scouting && league.settings.focus && (league.mode !== 'pro' || (league.settings.positions && league.settings.weather)) ? '' : html`<small class="muted">A league started before these existed keeps its old rules until you switch them on here.</small>`}
  ...
  if (league) {
    ...
    root.querySelector('#positions')?.addEventListener('change', (e) => ctx.update((st) => { st.league.settings.positions = e.target.checked; }));
    root.querySelector('#weather')?.addEventListener('change', (e) => ctx.update((st) => { st.league.settings.weather = e.target.checked; }));
