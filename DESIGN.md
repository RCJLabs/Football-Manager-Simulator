...
`npm run calibrate` checks nine numbers. That is enough to keep scoring sane and not nearly enough to catch a simulation that arrives at the right score by the wrong route. `npm run realism` simulates 400 games and prints forty measurements against the real league's range, marking anything outside it. Everything below was found by running it, not by reading the code — which is the point of writing it.
...
**What is still out.** Touchdowns per field goal reads 1.99 against a real 1.3 to 1.8: the scoring total is right but its composition leans a touchdown heavy and half a field-goal attempt light. Twenty-plus-yard plays sit at 3.3 against 3.5 to 5.5. Both are fixable only by pulling on knobs that move numbers currently in range, which is how a calibration gets worse while every individual change looks like an improvement, so they are recorded rather than chased.
...
Not built: a separate rookie draft — a generated rookie goes into the same market as everybody else. Aging and development *are* built and are the subject of the next section; this paragraph claimed the opposite for as long as it took somebody to read the two in sequence.
