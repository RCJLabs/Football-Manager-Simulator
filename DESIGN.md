...
One component serves both, because the shape is the same: clubs across the top, what they have taken stacked underneath. In a snake draft a row is a round and it lines up exactly, since every club picks once per round. In an auction clubs buy at different rates, so a row is the Nth man that club bought and a column is read down rather than across; cells there carry the price. A 32-club board is 27 rounds by 32 clubs, so it scrolls in both directions inside its own box — the page never scrolls sideways — with the round column and the club headings pinned, and `scrollToPick` nudging only the board's own scroller to keep the live cell in view. Cells show a surname, because a full name wraps to three lines at 5.6rem and a board that cannot be read at a glance is not a board. Ratings on it go through `shownOverall`, so an unscouted rookie is not given away here.
