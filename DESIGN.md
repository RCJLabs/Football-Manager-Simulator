...
quality, leverage-weighted: a standard deviation of 36.9 under the auction
against 21.1 from the draft, over the default twelve leagues.
...
game matched real carries 37.0 against 26.9, the one set once the passer and
the receivers were held to real seasons 37.0 against 21.9, and the one set on
the drive model 40.4 against 22.6, and 40.2 against 22.0 once the rating
weights were re-measured on it, and 33.0 against 21.1 once the lines were held
to real absences and the table and the weights re-set on them, and 35.6
against 20.4 once the pocket read the lines, and 36.9 against 21.1 once the
coverage made sacks: the auction now spreads rosters about three quarters again
more than the draft does, as on the one before, where before that it was half
again more, before that four fifths more, before that two-thirds more, before
that a third more and on the oldest half again more. The fall to 33.0 was the
table: weighed by it, the line that used to be the biggest spend had become the
cheapest starter, so the rosters that differed most by how much line they
bought came to differ least. With the pocket the lines are worth a little more
again, and so is the spread.
...
**A value ratio is meaningless when the stake is trivial.** Re-measured, the punter came out at better value-for-money than the quarterback — arithmetically true, and it had the value board telling a new manager that punters are the best buy in the game. `positionValue()` now carries each position's whole-squad stake and calls anything under 7.5% "barely matters" however good its ratio, measured on the group so five cheap linemen are not mistaken for a specialist. The line was 5% until the table set on the drive model put the punter at 4.99%, where half a hundredth of a point on his table weight, a fortieth of its standard error, would have made him the best buy on the board; the specialists stood at 4.5% and 5.0% and the next position at 10.0%, so the line sits in the gap. On the table set once the lines were held to real absences they stood at 4.8% and 6.1% and the next, the tight end, at 9.2%, on the one set once the pocket read them at 5.0%, 6.1% and 8.9%, and on the one set once the coverage made sacks at 4.8%, 5.9% and 8.5%: still the gap.

**Which positions are actually the bargains**, on the table as it stands (set on 2026-09-25 once the coverage made sacks; see "Coverage sacks, held to real absences"):

| | CB | OL | S | LB | QB | TE | DL | RB | WR |
|---|---|---|---|---|---|---|---|---|---|
| Wins share | 12.8% | 5.6% | 6.6% | 7.4% | 21.4% | 8.5% | 7.9% | 10.6% | 8.5% |
| Price share | 6.9% | 4.9% | 5.9% | 7.3% | 22.5% | 9.3% | 9.3% | 15.7% | 14.7% |
| Value | 1.86× | 1.14× | 1.12× | 1.01× | 0.95× | 0.91× | 0.85× | 0.68× | 0.58× |

Cheap is not the same as underpriced, and the table has changed its mind about which is which seven times in two days. On this one the corner is the one bargain, nearly twice what the room pays for him now that he makes sacks; the offensive line, the safety, the linebacker and the quarterback are about right, the quarterback right on the line at 0.95×; the tight end and the defensive line are overpaid; and the back and the receiver are the traps, the back just under the line at 0.68×. On the table before it the corner was already the one bargain at 1.63×, the linebacker overpaid at 0.85×, the back right on the line at 0.70× and the receiver the one trap at 0.50×. On the one before that the line read about right at 0.97× and the defensive line overpaid at 0.73×, on an engine whose lines reached the passing game only through pressure. On the one before that the offensive line was the first bargain at 1.72×, on an engine whose run game read the lines three to six times as far as the real one, and the defensive line and the quarterback were a little overpaid at 0.92. Until the passer was held to real seasons the quarterback was the first bargain at 1.48× and linemen came out overpaid at 0.96×: a point of a quarterback's rating moved his production 2.4 times the real game's, so his win share was inflated and every other position's was measured against it. For a few hours earlier the back was the one clear bargain at 1.69×, on a table that was right about an engine whose run game was wrong. The strategy table below is where each of these is tested against buyers who bid on something else.

One limitation to carry openly: this is an average across a position's starters, and where a position has several the first is worth more than the average. Measured solo over the same six rosters, a number one receiver is worth 1.20 against the positional average of 0.94, about a quarter more (1.01 against 0.76 on the table before, 1.00 against 0.77 before that, 1.01 against 0.75 before that and 0.91 against 0.69 before that). It was nearly twice (2.14 against 1.12) while a receiver nine points better drew 2.7 times the targets. The value panel says so.
...
|---|---|---|---|---|
| Value shopper (bid 1.15× true worth) | 7.6 | +35 | 3.9 | 3 |
| Stars and scrubs (2.4× asking for anyone rated 93+, $1 for the rest) | 7.0 | +4 | 4.7 | 4 |
| Trenches first (1.7× worth on the lines) | 6.8 | −6 | 4.7 | 3 |
| Skill players first (1.5× asking at QB/RB/WR/TE) | 6.3 | −28 | 5.3 | 3 |
| Market follower (pay the asking price) | 4.5 | −80 | 6.9 | 1 |
| Spread it evenly | 4.2 | −105 | 6.8 | 0 |

Re-run once the coverage made sacks and the table and the weights were re-set on it (see "Coverage sacks, held to real absences"). Value shopping still wins most, 7.6 where it read 7.8, and the three strategies behind it are within about a standard error of where they were. The two that buy what the room buys lost about a win each: following the market 4.5 where it read 5.7, spreading it evenly 4.2 where it read 5.3. That is about two standard errors of a difference each. Most likely it is the corner, whom the room now underpays by more than it did and neither of them buys on worth; that is not isolated.

Re-run once the pocket read the lines and the table and the line's weights were re-set on it (see "The pocket, held to real absences"). Value shopping still wins most, 7.8 again. Trenches first comes back to second, 7.3 where it read 6.8, with the lines worth about three tenths more than they were; following the market reads 5.7 against 5.2. Each figure carries about ±0.4, so the order at the top is the finding and the rest is inside a new draw's noise.

Re-run once the lines were held to real absences and the table and the weights re-set on them (see "The trenches, held to real absences"). Value shopping still wins most, 7.8 where it read 7.7. Trenches first falls from 7.5 to 6.8, level with stars and scrubs, which is the offensive line no longer being a bargain: the table now prices a lineman at the kicker's level, so paying 1.7 times worth for five of them buys less than it did. Following the market falls from 6.2 to 5.2, to the bottom. Each figure carries about ±0.4, so the order at the top is the finding and the rest is inside a new draw's noise.

Re-run once the rating weights were re-measured on the drive model (see "The rating weights, re-measured on the engine as it now is"), which changes who every club bids on: value shopping still wins most, 7.7 where it read 8.0, and every strategy is within about a standard error of the run before. Following the market reads 6.2 against 5.6.

Re-run before that once the drive was held to real play-by-play and the table set on it. Value shopping still wins most, but reads 8.0 where it read 9.1, and following the market reads 5.6 where it read 5.0. The table is not what moved it. On the same engine and the same 24 leagues the table before reads 8.6, and on 24 more leagues both tables read 8.0: 8.3 against 8.0 over the 48, where a difference like that carries about ±0.45. Whether the engine did it, or the 9.1 was a high draw, is not established either: the table before reads 8.3 on this engine against 9.1 on the last, about 1.4 standard errors. Over the 48 leagues the new table helped skill players first (6.75 against 5.9 on the table before) and trenches first (7.65 against 7.0) most, the second most likely the offensive line becoming the first bargain; stars and scrubs reads 7.6 against 7.25. None of those differences reaches two standard errors.

Before that, re-run on 2026-09-24 once the passer, the receivers and the backs were held to real seasons and the table re-set with them. Value shopping reads 9.1, the best it has measured; trenches first rises from 6.5 to 8.0 and ties it for titles, which is the offensive line becoming the bargain; stars and scrubs falls from 7.8 to 7.0, a star receiver no longer taking the ball from everyone else. Earlier the same day, once the run game was measured against real carries, value shopping read 8.8, skill players first fell from 6.4 to 5.3 and trenches first rose from 5.8 to 6.5, which was the back being worth a third of what he had been. Earlier the same day, paired on the same leagues, the table that had the back at nearly a quarterback took value shopping from 7.3 to 8.2 over the one before it — right about an engine that was wrong. Every correction to the worth table before the drive model made the strategy bidding off it better, which is what a more accurate table should do; the one set on the drive model made no measurable difference to it. Twenty-four leagues is a small sample and the ordering is stable across runs while the individual numbers move by a few tenths.

A win and a half to two and a half separate reading the market from following
it (four on the engine before the drive model), so how you bid is still the
main thing that decides a season.
...
carries, 113% on the one set once the passer and the receivers did too and
113% again on the one set on the drive model, 117% on the one set once the
lines were held to real absences, 115% on the one set once the pocket read
them, and 115% again on the one set once the coverage made sacks (`npm run cap
-- 1 4`), with the cap left where it was for the reasons under "Twenty-five
seasons in". A star quarterback at 87 costs 23, where he cost 25 on the table
before, 27 before that, 21 before that and 39 before the passer was fixed; a
corner at 87 costs 14 (12 on the table before); a lineman at 87 costs 6, where
he cost 5 two tables ago and 9 before the lines were held.
...
### What the invariant sweep found (`npm run sweep`)

The pro-auction bug above lived in a combination of setup choices that no test and no measurement had ever built, and it broke four places at once. `scripts/invariant-sweep.mjs` is the answer to that kind of bug rather than to that bug: it plays every combination worth telling apart — the two modes by the two ways of building a roster, one setting changed at a time around each (careers, injuries, difficulty, the smaller features, club count, keeper quota, cap room, coaching jobs, position changes), and six pairs that meet in the offseason's money — through five seasons with every decision left to the AI, and checks what must hold in any league at all: full rosters of the right positions, nobody on two clubs, nobody retired or drained on a roster, a contract for everybody carried and nobody else, a term on every capped deal, nobody over the cap, keepers within quota and priced as written, every AI keeper list passing the check the user's must pass, a market that completes, and — at every season's end, where a save is largest — a save that packs and unpacks to exactly what went in, at both levels the store packs at, and to under a third of the origin. The audit checks the numbers; this checks the rules. Forty-seven leagues, about five minutes on three cores.

The first run: every fantasy league clean, no exception and no stall anywhere, and every one of the twenty pro leagues broken — 171 readings in all, a man counted once for each season he stayed. Three causes, found by tracing rather than guessed:

- **A trade could sign a man the drain had shown out.** An uneven trade leaves a club a position short, and `backfillPlan` signs the best free agent to square it — from the raw pool, where free agency, the wire and the kickoff fill all sign from `signablePool`. In a pro league past its first season the raw pool still holds every all-time player the drain has retired from the market, so the backfill found them first. Traced in one league, all eleven drained men who reached a roster came in this way. The trade block's `bestAvailable` had the same pool, so it priced giving a player up against a replacement no club could sign.
- **The drain did not count injured reserve as a roster.** `enterOffseason` handed it the twenty-seven slots and nothing else, so a man on reserve with a drain date due was shown out, and `clearIr` two lines later put him back in his slot. Rare — two of 1,315 drained men over ten offseasons in two leagues — but it is the same broken rule. Reserve and the practice squad count now, through `teamContractIds`.
- **The AI chose its keepers on last season's players.** `enterOffseason` advances every career and then picks the AI's keepers and sets the tag's going rates — both, by their own comments, "on who a player is now" — but from the index it was handed, built before the careers moved. The keeper screen and `confirmKeepers` read the new one. So an expiring man who had improved was priced low by his own club and high by the league, and three AI lists in the sweep passed their own cap test and failed the real one. Both now read an index rebuilt after the careers and the class; the owners' review keeps the old one, since it judges the season that was played. It moves nothing a fantasy dynasty can measure: `npm run dynasty` reads 5.0 of 6 kept, $45 committed, keepers at 91.1 and 66% turnover before and after.

After the three fixes: forty-seven of forty-seven clean. A test for each fails on the old code (`tests/league-invariants.test.js`), and the sweep run against the old engine finds ten of these in one pro league, so it would have caught them.
...
### A man let go took his deal to whoever signed him next

Only the kickoff's cut to the cap ended the contract of the man it let go. Every other way off a roster left the deal in `league.contracts` — a waiver claim's drop, a release from reserve or the practice squad, the man displaced to activate or promote another, the spare man in an uneven trade — and it sat there until the next offseason's sync cleared it. A club that claimed him in the meantime took it over: the old salary and the old term, while the club that dropped him was paying dead money on the same deal. Counted over four kinds of league (pro and fantasy, drafted and auctioned), two seeds and three seasons each, **172 of 1,342 waiver claims** landed a man on the deal of the club that dropped him — 73 of 533 in a drafted pro league, 87 of 644 in an auctioned one, 3 of 77 and 9 of 88 in the fantasy ones — carrying $10 a year over the minimum in the drafted pro leagues and $115 in the auctioned ones. The rest of those claims were of men who had no deal to take.

`endContract` in `cap.js` states the rule once: a man off a roster for good takes no deal with him, and his contract ends where he leaves, after `bookDead` has charged whatever is still owed. Every path that lets a man go calls it or, in `injuries.js` and `squad.js` (which `cap.js` imports), does what it does. The same count afterwards: **0 of 1,344**.

Ending the deal where he leaves exposed the second half. A man with no deal is given one by `syncContracts`, which signed anybody named in the season's draft or auction on its terms — the pick's rookie deal, or what the room paid — whichever club had drafted or bought him. With deals ending on release, 25 men across the same leagues were on a roster without one when a season ended, all of them named by a market for some other club, and all 25 would have been signed on that club's terms. The market's terms are now written only at the kickoff a draft or an auction has just filled (`startSeason` reads that off the phase it starts from); at any other sync — a season's end, or a season run back on the same rosters, which starts from `complete` — a man without a deal arrived off the street and is signed on the minimum. After the change none of the 25 is on market terms. A club that lets its own draftee go and claims him back is off the street too, rather than handed a fresh four-year rookie deal.

`tests/leaving.test.js` holds each path, both syncs, and the fantasy auction's version (a claimed man kept at the $1 the dynasty section promises, not the price his old club paid); each test fails on the old code, and taking out any one of the six release paths, or getting either sync wrong in either direction, fails at least one.
...
**What the other clubs have left.** The keeper screen's table of the other clubs showed each one's cap left as the cap less its keepers. `validateKeepers`, which decides whether a list stands, also takes off what a club still owes men it has let go, so a club carrying dead money read that much richer than it was — and that column is what a player reads to guess who can afford whom in free agency. It takes it off now, and says so under the table. The user's own panel took it off when the list was valid, but not in the figure it shows while the list is over the cap, and it never showed the amount, so its lines did not add up to the cap; it shows it now.

**The way out.** In a capped league the keeper round closes into free agency, but the button said it opened the draft (or the auction) and sent the page to `#/draft`, which sent it to the season hub, which sent it back to `#/offseason` — three screens and three history entries for one tap. It names free agency now and draws it in place, at the top of the page. The season hub's champion card said *keepers, then the draft* in a pro league as well, and now puts free agency between them.

`scripts/smoke.mjs` checks both in a browser, on the injected pro league with money owed by one of the other clubs and by the user: every club's cost and cap left come to the cap once what it owes is counted, the panel shows what the user owes and what is left, the button names free agency, and confirming from partway down the page reaches free agency with no change of address and at the top. Run against the old code it reports seven failures: the table and its missing note, the panel, the button, the three hops, and free agency's two stale lines.
...
**That paragraph stopped being true at the two-pool split**, and the screen went on saying it. Free agency is veterans — `signablePool` keeps this year's rookies for the draft, or for a pro league's auction — so nobody here is in the draft an hour later, and a bid buys a man the market will not otherwise hand you rather than the certainty of one it would. What a man nobody signs does is wait: a slot still empty after the draft is filled at kickoff from whoever is left, on the minimum for a year. The pick still holds for most clubs but not all, because a club drafts once a round only while it has a slot open. Measured when the market opens, over 384 pro club-offseasons (two seeds, three offseasons each, drafted and auctioned): a median of four slots open against a five-round draft, two at the tenth percentile and seven at the ninetieth, so for three clubs in four every signing is a pick not made, and for the rest the first few replace men the kickoff fill would have signed. In an auction league the cost is the cap money and the slot the rookie auction would have had. The screen says this now, and so do `freeagency.js` and `confirmKeepers`; its close button, which said *draft* in an auction league too, names the right one, and the note after the market closes no longer says whoever is left is in the draft.
...
| --- | --- | --- | --- | --- |
| QB | **r = 0.979** | 500 | 0.005 | 9.4 |
| CB | **r = 0.957** | 500 | 0.034 | 5.8 |
...
*The quarterback and corner rows were re-run on the drive model: they read
0.988 over 22.0 points and 0.984 over 7.7 when this table was made. The
quarterback's range fell 2.4 times, the same factor by which a point of his
rating had moved his production too far before the passer was held to real
seasons. The corner read 0.966 on the drive model and 0.975 once his weights
were re-measured on it. Once the lines were held to real absences the corner
reads 0.951 over 4.7 points and the quarterback 0.978 over 9.5; the corner's
fall is the engine, not the weights, which did not change: the commit that
held the lines took him there with the old table and weights, his range
narrowing from 5.6 points, most of it in what his own side scores (0.914 over
2.2 to 0.823 over 1.6), while against the opponent's points he still reads
-0.972. Once the coverage made sacks and the corner's weights were re-set on
it, the corner reads 0.957 over 5.8 points, where he read 0.954 over 4.5 once
the pocket read the lines, and the quarterback 0.979 over 9.4: a corner who
makes sacks is worth more to his side, and against the opponent's points he
now reads -0.975 over 4.5. The other rows, and the swing column, were not
re-run.*
...
seasons, and again at 8.68 once the drive was held to real play-by-play; the
yardstick column is still the stale one.*
...
done. On the drive model the real corners read 0.966 and the generated ones
0.958, and the weights were then re-measured on it (see "The rating weights,
re-measured on the engine as it now is"): ball skills 0.19, coverage 0.51,
tackling 0.05, and the two read 0.975 and 0.968. Once the lines were held to
real absences they read 0.951 and 0.961, once the pocket read them 0.954
and 0.944, and once the coverage made sacks and the corner's weights were
re-set on it 0.957 and 0.963, each step inside the swing a man carries at 500
games.*
...
|---|---|---|---|---|
| low | 0.67 | 0.60 | 0.17 | 0.7% of weeks |
| normal | 1.31 | 1.12 | 0.34 | 2.8% of weeks |
| high | 2.57 | 2.07 | 0.72 | 5.7% of weeks |

*Re-run once the lines were held to real absences, and the two left-hand columns
over a thousand games rather than three hundred. They had read 0.52, 1.15 and
2.63 injuries a game since they were first measured, on an engine with fewer
snaps a game; at three hundred games the default carries about 0.07 of noise,
and a change that re-drew the random stream moved it from 1.20 to 1.39 with
nothing in the injury model touched (1.29 and 1.26 over a thousand games either
side of the change). The QB1 column is ten seasons of one league and moves by a
point or so with every set of drafts. Re-run again once the pocket read the
lines, on the drafts of the table set on it: 0.67, 1.34 and 2.59 became 0.66,
1.30 and 2.54, and the QB1 column went 3.0%, 2.6% and 5.3% to 1.3%, 4.5% and
5.1%. And once the coverage made sacks, on the drafts of the table and the
weights set on it: 0.67, 1.31 and 2.57, and the QB1 column 0.7%, 2.8% and
5.7%.*
...
|---|---|---|---|---|---|
| fantasy | low | 14.0 | 2.4 | 0.10 | 0.09 |
| pro | low | 17.0 | 2.5 | 0.21 | 0.09 |
| fantasy | normal | 14.0 | 4.7 | 0.39 | 0.14 |
| pro | normal | 17.0 | 5.4 | 0.54 | 0.20 |
| fantasy | high | 14.0 | 10.0 | 0.80 | 0.38 |
| pro | high | 17.0 | 11.6 | 1.12 | 0.38 |
...
evidence is weaker than it was. Re-drawn again on the drive model and the table
set on it, they read 1.18, 1.16 and 1.15, and all three agree again. The
starter-weeks rose about a seventh in both modes on that draw (fantasy normal
4.4 to 5.0, pro 5.1 to 5.8), and this said it was the engine. It was mostly the
draw: re-drawn once more on the rating weights re-measured on the drive model,
they read 4.4 and 5.4, ratios 1.18, 1.23 and 1.17. A game has about 5% more
snaps than it had and injury risk is per snap, which is the part of any rise
that is the engine; the rest moves by half a starter-week a draw. Re-drawn on
the table set once the pocket read the lines, the ratios read 1.13, 0.98 and
1.26: this time the normal row, fantasy up from 4.5 to 5.3 and pro down from 5.7
to 5.2 on the same draw. The risk is per snap in both modes by construction, so
a ratio under one is the draw, and it is the thinnest evidence this table has
given. Re-drawn once the coverage made sacks, on the drafts of the table and
the weights set on it: 1.04, 1.15 and 1.16, the normal row back above one
(fantasy 4.7, pro 5.4) and low the thin one again.*
...
|---|---|---|---|---|---|
| fantasy | 64 | 93.9 | 90.6 | 3.2 | 1.5 |
| pro | 256 | 90.0 | 84.3 | **5.7** | **3.7** |

Both ends are diluted and the backup end far more. The spread matters as much
as the mean: at sd 1.5 a fantasy club is nearly always well covered at
quarterback, while at sd 3.7 a pro club's cover is a lottery. (The fantasy
spread has read 1.4, 2.7, 1.7 and 1.5 on the drafts of four successive tables
and weights; the drafts, not the pool, move it, and the pro row has not
moved.) Measured over
...
**Games read it as the rosters stand.** The bonus used to be cached, one league at a time, keyed by the league object, its `tenure` object and the player index — and chemistry reads each club's starters and how long it has held them, which a trade, a waiver claim or a new depth chart changes without replacing any of the three. `tenure` is replaced once a season and the app's index only when careers, rookies, retirements or position moves change it, so a season's games were played with the chemistry of the first game after the page loaded, a reload in mid-season changed it, and this screen, which reads it fresh, could show one thing while the games used another. It also made a stretch's results depend on what else had been simulated in the page (see *A run the page can draw through*). Played the way the app plays a season — an index held all season, the season simulated through — a club's real bonus moved as far as 1.04 rating points from the kickoff reading the cache kept using, against a cap of 1.8; the median week's worst club was 0.39 off. Reading the whole league costs 76 µs, about a millisecond a week, so games now read it fresh and nothing is kept. Results move from the first week a roster does: across three pro seasons 703 of 855 games came out with different scores, which says how chaotic a season is once anything changes rather than how much chemistry is worth. The audit agreed on all 41 checks; two moved by a tenth within their tolerance, both season-long injury measures (a club missing 0.35 at the default setting, a pro club losing 5.5 over a season), and the rest were unchanged. `tests/chemistry.test.js` holds that a game is played with the chemistry the rosters have now, and that a stretch of games comes out the same whatever else was simulated first; both fail on the old cache.
...
### A run the page can draw through

Played straight through, a run held the page from the press to the end: nothing on screen moved and every tap queued behind it. Measured through the real buttons on a 32-club pro league, with Chromium's CPU slowed four times to stand in for a mid-range phone:

| | took | longest without a frame |
| --- | --- | --- |
| to the halfway point, the first run of a session — before | 26.2 s | 25.9 s |
| the same — after | 26.6 s | 3.6 s |
| into next season from week 10 — before | 9.9 s | 9.5 s |
| the same — after | 8.1 s | 2.1 s |

The run takes as long as it did; the page now draws after every step. `simulateSteps` is the run as a generator, yielding after each week, each playoff round and each stage of the offseason — opened, a job taken after a sack, keepers, free agency, the draft or auction — and every one of those leaves the league at rest, where the player could have stopped. `simulateAhead` runs it straight through, so scripts, tests and the sweep see nothing different; `simulateAheadAsync` pauses between steps, and the page's pause waits for a frame. `ui/simulate.js` runs it behind a panel that says where it has got to and makes the rest of the page inert, and writes the league once, at the end: a save between steps would cost a phone a serialization of the whole league every week, and a run cut short by the page closing is better lost whole than kept in part. The panel's bar is a CSS transform, which Chromium animates off the page's thread, so it should keep moving through a long step; that part is expected rather than measured.

Stepping had to change nothing, and it does not. The released `simulateAhead` was run against both new runners, each in a module instance of its own, from the same league and random state, through every target, for pro and fantasy leagues built by draft and by auction, over two offseasons: the leagues, the account of the run and the random state came out byte for byte the same. `tests/autosim.test.js` holds the two runners to each other, and the smoke's check of the panel — a frame drawn for every week while it is up, a line saying where the run has got to, nothing underneath usable — fails on the old screen (no frames across fourteen weeks, no progress) and passes on the new one (fourteen frames, fourteen lines).

**Why a step can still take 3.6 seconds.** A step is a week: its games, 40–140 ms here with no slowdown, and then its moves, 280–710 ms before the trade deadline and under 100 after it. The moves are almost all the AI's trade search, which checks each uneven deal it considers with four `backfillPlan` calls, and each of those rebuilds the map of who owns whom (every rostered man, 32 clubs of them) and filters the whole pool again. That is also what the ordinary Advance button costs every week before the deadline, which this does not change.

**The trade search now asks once.** Nothing in `makeAiOffers` changes a roster — offers are made, not done — and `aiTrades` changes two only when it executes a deal, so the questions the search kept asking have the same answers until then. `tradeSearch` holds them: the owner map, and for each position the signable men available, best first, built with the same filter and the same stable sort from the same order as the uncached path, plus each club's backfill plan and the roster it leaves, keyed by the deal's exact shape. `makeAiOffers` keeps one for the whole call; `aiTrades` starts a new one after each deal it executes; the deal finder starts one per club scan, because the screen can move a roster between scans but not during one. Everything else — the player's own trade builder, a proposal, an accepted offer — asks afresh as before.

| | before | after |
| --- | --- | --- |
| a pro week's moves before the deadline, no slowdown | 280–710 ms | 63–192 ms |
| the Advance button, week 1, CPU slowed four times | 3.1 s | 1.0 s |
| simulating to the halfway point, the same | 26.6 s, longest without a frame 3.6 s | 7.1 s, 1.1 s |
| into next season from week 10, the same | 8.1 s, 2.1 s | 5.7 s, 0.8 s |
| the deal finder's whole scan, no slowdown | 820 ms | 222 ms, the same four deals |

The results did not move. The released engine against this one, each in a module instance of its own, from the same leagues and random states through every target and two offseasons, gave byte-identical leagues, and those runs held 52 AI trades (37 uneven) and 136 offers to the human (115 uneven), so the cached path was exercised rather than skirted. `tests/tradesearch.test.js` puts every deal a club weighs against the user in a second-season pro league — 4,464 candidates, 561 of them uneven deals that pass, 1,122 rosters squared — through the plain path and a shared search and compares every answer; the best free agent at each position is retired and the next drained first, so the market's exclusions are tested rather than assumed. Four of five mutations of the cache fail it. The fifth, dropping the club from the plan's key, cannot collide in a real search, since a deal's players belong to one club; the key keeps it anyway. One thing no run exercised: `aiTrades` starting a fresh search after a deal. None of these leagues had a later pair in the same week wanting a man an earlier deal had just signed. It is required regardless — the test shows a kept search still offering the man a deal just signed — and it is in.

**Found on the way, and not fixed here.** Proving the runners identical first seemed to show they were not: with two copies of one league simulated in turn in one module instance, a club had six wins by the playoffs in the run on its own and seven in the shared one. The chemistry cache in `season.js` is the cause — with it switched off in both the old and new code the difference goes away. It holds one league's bonuses at a time, keyed by the league object, its `tenure` object and the player index, so the chemistry a game is played with depends on what was computed before it in the same page, not only on the league. How often that changes a result in one league played normally is not measured. Fixing it changes results, which the audit measures, so it is a change of its own. *(Since fixed: chemistry is read fresh for every game — see Team chemistry.)*
...
(see "Backs, receivers and tight ends, held to real seasons"), then to 8.68
on the drive model.* The read, the
...
dropback (5.93, ±0.3). They read 1.02 and 6.63 when this was written; the drive
...
now reads. On the drafts of the table and the weights set on it, 5.93, the
tenth or so two leagues carry either way, and 5.93 again once the coverage
made sacks, with the quarterbacks' slope at 1.02.
...
the audit reads the receivers' target-share slope (0.82, flagged outside ±0.2)
and the backs' carries slope (1.02, ±0.25). From `npm run rushing` it reads the
backs' slope (0.93, flagged outside ±0.15) and a pro league's yards a carry
(4.59, ±0.25). They read 0.80, 1.13, 0.88 and 4.57 when this was written; the
drive model held to real play-by-play (next section) moved them, the last to
4.55, and the rating weights re-measured on it moved that to 4.58. Holding the
lines to real absences moved them to 0.82, 0.86, 0.93 and 4.59: the carry reads
the two lines at a quarter of what it did, so a back's own terms carry more of
his yards, and the play caller's lean on the run by the quarterback's rating
less the back's comes true a little less. Once the pocket read the lines the
carries slope read 0.96 on the new drafts, a draw's worth, and the others
stayed where they were; and once the coverage made sacks, 1.02 on the drafts
of the table and the weights set on it, with the receivers at 0.82 and the
backs at 0.95 and 4.61 yards a carry.
...
**The value table, set again** on six rosters at 10,000 games each, a starter
eight points better:

| | QB | TE | RB | CB | DL | OL | LB | S | WR | P | K |
|---|---|---|---|---|---|---|---|---|---|---|---|
| per starter, points a game for +8 | 2.38 | 1.21 | 1.14 | 1.14 | 0.99 | 0.97 | 0.96 | 0.84 | 0.75 | 0.57 | 0.51 |
| the table before | 9.02 | 4.06 | 3.90 | 4.52 | 3.90 | 3.19 | 3.57 | 3.28 | 2.72 | 1.98 | 1.50 |
| the table now | 8.68 | 4.43 | 4.17 | 4.15 | 3.60 | 3.53 | 3.49 | 3.08 | 2.75 | 2.09 | 1.87 |

The quarterback is 8.68, ± 0.43 between rosters, where he was 9.02: a point
of his rating is 0.30 points a game against the real game's expected-points
estimate of about 0.28. The tight end, the back and the corner are within a
standard error of one another. Beyond their noise, the kicker rose (1.50 to
1.87), the offensive line rose (3.19 to 3.53), and the defensive line (3.90 to
3.60) and the corner (4.52 to 4.15) fell. None of the four was isolated; the
kicker's is most likely the real kicking curve, on which a stronger leg's
yards reach tries from 50 to 60 that the logistic had all but written off. A
table measured one change earlier, before the late-game mode and the red zone
squeeze, came out within noise of this one (QB 8.47, CB 4.21, TE 4.21, RB
4.17), so those two did not move it.

The value board's line for a position that cannot matter moved with it. The
punter came out at 0.0499 of the board against a line of 0.05, half a
hundredth of a table point, measured to ± 0.18, from being called the best
buy on the board. The specialists stand at 0.045 and 0.050 and the next
position at 0.100, so `MATTERS_AT` is 0.075, in the gap rather than at the
edge of one side of it.

**What moved with it**, against the table before:

| | before | now |
|---|---|---|
| value board: underpaid / overpaid / badly overpaid | CB, OL / none / RB, WR | OL, CB / DL, QB (0.92) / RB, WR |
| value shopper, wins of 14, the default 24 leagues | 9.1 | 8.0 |
| value shopper on the table before, this engine, 24 leagues / 48 | — | 8.6 / 8.3 (new table: 8.0 / 8.0) |
| trenches first / stars and scrubs, 24 leagues | 8.0 / 7.0 | 7.2 / 7.4 |
| AI offers a league-season / weeks with one | 10.6 / 49% | 10.3 / 48% |
| offers that help the human | 46% | 41% |
| a roster at market against the cap, founding | 113% | 113% |
| roster spread, auction / draft | 37.0 / 21.9 | 40.4 / 22.6 |
| a star quarterback at 87, of a 200 cap | 22 | 21 |
| MVPs in 40 seasons, the table read straight | QB 33, DL 6, CB 1 | QB 37, DL 2, CB 1 |
| MVPs in 40 seasons, as awarded | QB 33, RB 7 | QB 33, RB 7 |
| the pass/run read, 0.70 against 0.55 | +0.68 to +1.04 | +0.54 to +0.71 |
| fourth-down aggression at the top, eight clubs / pro | +0.71 / +0.82 | +0.37 / +0.14 (+0.39 / +0.31 at 0.8) |
| blitzing least | +0.33 / +0.23 | +0.41 / +0.50 |
| the deepest shell | +0.38 / nothing | +0.36 / nothing |
| starter-weeks lost to injury a club-season, fantasy / pro, normal | 4.4 / 5.1 | 5.0 / 5.8, a high draw (4.4 / 5.4 on the next) |
| a fantasy club's backup quarterback, gap / spread | 3.3 / 1.4 | 3.5 / 2.7 |
| a replacement-level quarterback, points of win probability | 34 | 39 |
...
- A punt from the opponent's thirty-one to forty on fourth and seven or more is
  ...
  opponent's thirty-one and fifty is gone for 47% of the time against a real
...
- Weather still moves a field goal by yards of distance. On the real kicking
  ...
  make chance than they did on the logistic, so a snowy gale gives up about
  half the long tries (53 to 56% over 1,200 games), where it gave up two
  thirds, and costs nine or ten points of makes from 40 to 47 yards, where it
  cost sixteen. Its constants were never set
  against real games, and are not reset here.

### The rating weights, re-measured on the engine as it now is

`overall` prices every player from one weight vector a position
(positions.js), and each vector was last moved toward the field on an engine
since rebuilt under it: the run game held to real carries, the passer and the
receivers held to real seasons, the drive held to real play-by-play. The
corner's ball skills had been noticed at 21% of his value against an 11%
weight; nothing else had been checked.

**Every position had drifted.** `npm run attrs` at 40,000 games a reading, the
punter at 160,000 because 40,000 left his accuracy at 1.4σ. Shares of each
position's leverage, with the shipped weight after the slash:

| position | measured / shipped |
| --- | --- |
| QB | awareness .38 / .35, accuracy .33 / .40, mobility .15 / .08, arm .14 / .17 |
| RB | speed .36 / .30, receiving .22 / .10, awareness .17 / .19, elusiveness .15 / .19, power .08 / .14 (2σ), ball security .02 / .08 |
| WR | catching .38 / .26, speed .34 / .30, routes .17 / .25, after the catch .11 / .19 |
| TE | blocking .47 / .25, catching .24 / .25, routes .18 / .20, speed .08 / .10, after the catch .04 / .20 (1.4σ) |
| OL | run blocking .57 / .45, pass blocking .34 / .45, awareness .10 / .10 |
| DL | pass rush .49 / .61, run defence .32 / .20, awareness .16 / .07, tackling .03 / .12 |
| LB, off the ball | run defence .34 / .23, coverage .19 / .23, tackling .18 / .27, awareness .17 / .13, rush .06 / .05, speed .05 / .09 |
| LB, on the edge | run defence .31 / .19, pass rush .31 / .45, tackling .18 / .17, awareness .14 / .08, speed .06 / .11; coverage below nothing (−5σ) |
| CB | coverage .51 / .57, ball skills .19 / .11, speed .16 / .13, awareness .09 / .11, tackling .05 / .08 |
| S | coverage .32 / .34, ball skills .20 / .12, run defence .17 / .12, speed .12 / .14, tackling .10 / .24, awareness .09 / .04 |
| K | power .50 / .40, accuracy .50 / .60 |
| P | power .85 / .69, accuracy .15 / .31 |
...
**The rule, and where it needed tightening.** A position moves as far toward
the measurement as the record allows (see "Two authorities, and where they
disagree"), and `npm run blend` now walks that blend in eighths against the
record. Its first reading said the edge vector could go the whole way for
free. It could not: that took Derrick Thomas from 88 to 84, Kevin Greene from
86 to 82 and DeMarcus Ware, Micah Parsons and Justin Houston three points each,
every one of them inside `legacy-check`'s three-point tolerance and so
invisible to its count. The tolerance is right for one man, whose rating is an
editorial estimate, and wrong for a reweight, which moves an archetype
together. So the blend also counts every point a position's recorded seasons
sit below their floors, and a step is taken only when that grows by a point
at most, one man's rounding. Even half the edge move cost a point: Terrell
Suggs rates 85.56 on the shipped vector against a bar of 86, and the test that
guards the twelve rushers the linebacker work brought up to their bars caught
him. The record's objection is to an edge rusher's pass rush weighing less at
all, not to the rest of the vector, so the rush holds at 0.45 and speed, run
defence and awareness move the measurement's way.

**A second check, against production.** The measurement lifts one attribute
eight points on a side of 82s; `overall` has to rank men across the whole
range. So each change was also scored on eighty generated players a position
(sixty for the kickers), whose attributes scatter independently as a
dynasty's do: each man's production measured once, playing in the same side,
then correlated with his rating under each vector, with a bootstrap over the
men for the difference. Lifting one starter alone rather than every starter
(`npm run attrs -- 80000 WR1`, new) read the same as the group lift at
receiver, corner and safety within 0.06 on every attribute. At linebacker it
put coverage at
0.33 rather than 0.19, since the best-covering backer takes the tight end and
the back. Both linebacker vectors improved the fit alike, and the group's is
the one taken.

**What moved.**

| position | blend | now | the record | generated players' fit |
| --- | --- | --- | --- | --- |
| CB | all | cov .51, bal .19, spd .16, awr .09, tck .05 | no change | 0.984 → 0.988 (+0.004, 95% 0.002 to 0.006) |
| LB, off the ball | all | rsd .35, cov .19, tck .18, awr .17, prs .06, spd .05 | no change | 0.968 → 0.972 (+0.004, 0.001 to 0.008) |
| LB, on the edge | all but the rush | prs .45, rsd .23, tck .17, awr .09, spd .06 | no change | 0.960 → 0.964 (+0.004, 0.002 to 0.007) |
| DL | an eighth | prs .60, rsd .21, tck .11, awr .08 | Gastineau a point further under | 0.930 → 0.933 (+0.003, 0.002 to 0.004) |
| K | all | kac .50, kpw .50 | one fewer under | 0.967 → 0.971 (+0.004, −0.001 to 0.010) |
| P | all | ppw .85, pac .15 | no change | 0.974 → 0.978 (+0.003, −0.002 to 0.008) |

The kickers' intervals touch zero, and both go anyway: the punter's own
sample, fitted by least squares, splits 0.84 and 0.16, the measurement's
0.85 and 0.15 on independent games, and the kicker's 0.53 and 0.47.

*Corrected once the lines were held to real absences (next section): the
intervals in this table are far too narrow to mean what they say. They resample
the men and not the games, and each man's production was measured once, over
600 games, where it carries about half a point of noise. Measured again on
independent games the same comparison moved by as much as 0.04, at a position
whose interval here would have excluded that. Every gain above sits inside that
noise, so the table shows neither that these moves helped generated players nor
that they hurt them, and the same goes for the defensive line's 0.954 below.
The moves stand on the field's measurement and the record, which is what the
rule asks; the check is now run on production averaged over five independent
sets of games.*

**What did not, and why.**

- **The defensive line** stops at an eighth. A quarter costs Dwight Freeney and
  more costs Mark Gastineau, both pass rushers. The whole measured vector would
  have lifted the generated fit from 0.930 to 0.954, the largest gain on
  offer, so this is the two authorities plainly disagreeing: the engine says
  a lineman's run defence is worth two thirds of his rush, the record's great
  linemen say otherwise, and the record wins by the rule.
- **The quarterback** stays. An eighth costs no named man but adds four points
  of shortfall across twelve quarterbacks, and further costs Lamonica, Layne,
  Namath and the pre-merger pocket passers. The full measured vector would
  lift the generated fit by 0.003.
- **The running back** stays. An eighth is free and changes nothing measurable
  (−0.0003); a quarter costs Hornung and then the power backs.
- **The tight end and the offensive line** have no free step. The tight end is
  the recorded disagreement above (Brock Bowers at every step). The line costs
  Ron Mix at every step, and "run blocking is worth 1.7 times pass blocking"
  is decisive about this engine (40σ) and contentious about football, the
  combination that stopped the tight end.
- **The receiver and the safety** measured a long way off, 0.32 and 0.36 from
  the shipped weights summed over their attributes, twice over (the group lift
  and the single starter). The record allows the receiver's whole move and the safety's
  three quarters, and neither improves the generated players' fit: the
  receiver −0.003 (−0.014 to 0.008), the safety −0.001 (−0.004 to 0.002), whose
  interval rules out a gain the size of the corner's. Fitted by least squares
  on the same men, a receiver's speed is worth nothing independent of the rest
  and a safety's tackling is his largest attribute, the opposite of what the
  lift says in both cases. The likeliest reading is that some of these effects
  are not straight lines across the range the lift does not visit (the speed
  terms are clamped at the defence's speed), which was not tested. Two readings
  that disagree are not a price, so neither is taken; the receiver's weights
  are the ones the record allowed last time, and his fit is the weakest in the
  game, 0.85, for reasons this did not find. *Corrected in the next section:
  what held them was the noisy check above. Averaged over 3,000 games a man,
  the receiver's whole move lifts the generated fit by 0.009 (0.004 to 0.016)
  and the safety's five eighths by 0.002 (0.001 to 0.004), and both have now
  moved.*

**What it does to ratings.** Real players' attributes run together, so they
barely notice: no one moves more than three points and most move none. The
ball-hawking corners rise (Trevon Diggs 3, Asante Samuel and Marcus Peters 2)
and the shutdown corner who rarely picked a ball off falls (Nnamdi Asomugha 2);
big legs rise at kicker and punter (Sebastian Janikowski 2, Todd Sauerbrun 3)
and accurate short legs fall (Matt Stover and John Carney 2); a coverage
linebacker falls (Telvin Smith 2) as run stoppers gain a point. The record
still reads two violations, Csonka and Riggins. Generated players, whose
attributes scatter and who make up most of a long dynasty, are where the
weights decide who is worth what, and that is where the fit improved.

**What moved with it.** Every club drafts and bids on `overall`, so the
drafts came out differently and everything read from a drafted league was
re-drawn:

| | before | after |
| --- | --- | --- |
| the starting kicker's leg, founding pro leagues | 87.3 | 87.7, so weather takes its kicking means at 88 |
| roster spread, auction / draft | 40.4 / 22.6 | 40.2 / 22.0 |
| a roster at market against the cap, founding | 113% | 113% |
| AI offers a league-season / helping the human | 10.3 / 41% | 7.3 / 35% |
| value shopper, wins of 14, 24 leagues | 8.0 | 7.7 |
| a pro league's adjusted net yards a dropback / yards a carry | 6.04 / 4.55 | 6.13 / 4.58 |
| fourth-down aggression four-fifths up, eight clubs / pro | +0.39 / +0.31 | +0.16 / +0.33 |
| starter-weeks lost to injury, fantasy / pro, normal | 5.0 / 5.8 | 4.4 / 5.4 |
| corner's overall against production, shipped / generated | 0.966 / 0.958 | 0.975 / 0.968 |

Most of that is a new draw rather than a new game: the AI's offer count has
read between 7.3 and 10.6 on the last four drafts, the aggression dial moved
about a standard error (the dials section above has it), and the injury row
showed the previous draw's rise to have been mostly the draw. The corner's
yardstick is the one number here the weights were meant to move, and it moved
on both populations. The weather constant moved because the leg did, as its
audit check asks: kickers are now rated on leg and accuracy alike, and clubs
draft accordingly.

Five tests failed on the first version and none was a regression in the
game. The edge rushers' test caught Suggs, which changed the edge vector. One
had its premise moved: linebacker to corner was the example that the group
rule, not the guess bound, turns a conversion away, and with the corner's ball
skills at 0.19 the guess bound turns it away on its own, so the test now shows
the rule with linebacker to safety. The other three were defects in the tests
that new drafts happened to expose, each fixed in the test and passing on both
sets of drafts: the deal finder's compared a deal worth exactly a tenth, which
floating point makes 0.0999…, against a floor of a tenth, where the finder
itself works in tenths; the coaching test forced 1-16 seasons but finished the
club thirteenth, inside the playoff line, so whether that was a miss depended
on where the draft ranked his roster; and the gale test still expected field
goals from 40 to 47 yards to fall by sixteen points, as on the kicking curve
the drive model replaced, where over 1,200 games they fall by 0.100 (0.092 on
these weights), so a bound of seven points passed on one draw of 200 games and
...
**The value table, a fifth time.** Six rosters at 10,000 games each, as
before. Per starter, in raw margin, against six rosters measured on the drive
model: a lineman went from 0.96 to 0.48, a linebacker from 0.94 to 0.62, a
defensive lineman from 0.97 to 0.68, a tight end from 1.14 to 0.92 and a safety
from 0.86 to 0.75. Nothing else moved outside its noise. The table keeps its
starter-weighted total, so the room they left went to everybody else:

| | QB | RB | CB | TE | WR | S | DL | LB | P | K | OL |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| before | 8.68 | 4.17 | 4.15 | 4.43 | 2.75 | 3.08 | 3.60 | 3.49 | 2.09 | 1.87 | 3.53 |
| now | 11.16 | 5.37 | 5.32 | 4.28 | 3.55 | 3.48 | 3.16 | 2.90 | 2.84 | 2.23 | 2.21 |

The quarterback is 11.16 ± 0.64 between rosters. The offensive line is now the
cheapest starter in the game, level with the kicker. That is what this engine
says, and the absences say it is short: a real lineman's absence costs his side
0.80 points a game (0.47 to 1.21) and the engine's 0.48, and the shortfall is
in the passing effect recorded below, not the run. The run effect that used to make up for
it is gone. So the table is right about the engine, and the engine is light on
the line.

**The rating weights on this engine.** `npm run attrs` at 40,000 games a
reading on every position the lines reach, and the receiver:

| position | measured / shipped |
| --- | --- |
| OL | pass blocking .65 / .45, awareness .18 / .10, run blocking .17 / .45 |
| DL | pass rush .66 / .60, run defence .15 / .21, awareness .12 / .08, tackling .06 / .11 |
| LB, off the ball | coverage .28 / .19, tackling .23 / .18, awareness .16 / .17, run defence .16 / .35, rush .10 / .06, speed .07 / .05 |
| LB, on the edge | pass rush .52 / .45, run defence .18 / .23, tackling .14 / .17, awareness .08 / .09, speed .08 / .06 |
| S | coverage .36 / .34, ball skills .19 / .12, speed .14 / .14, tackling .13 / .24, awareness .12 / .04, run defence .06 / .12 |
| TE | catching .32 / .25, blocking .31 / .25, routes .17 / .20, speed .13 / .10, after the catch .06 / .20 (1.9σ) |
| RB | speed .34 / .30, awareness .19 / .19, receiving .19 / .10, elusiveness .15 / .19, power .11 / .14, ball security .02 / .08 |
| WR | catching .38 / .26, speed .33 / .30, routes .20 / .25, after the catch .09 / .19 |

The line turned over: run blocking at 1.7 times pass blocking became pass
blocking at 3.8 times run blocking, the way the real game pays its linemen. The
off-ball linebacker's run defence fell from the largest thing he does to a
sixth of him.

**The check against production had to be fixed first.** It scores a vector by
how well it ranks eighty generated players by what they produce, and it had
measured each man once, over 600 games. Run again on independent games it
moved by as much as 0.04: the line's whole move read +0.037 on the first set and
-0.001 on the second, where its bootstrap had called the first 0.016 to 0.061.
The bootstrap resampled the men and not the games, and a man's production over
600 games carries 0.4 to 0.7 of a point of noise. So every position here was
played five times over, 3,000 games a man, which leaves about a quarter of a
point, and the interval is taken on the average:

| position | step | generated players' fit, against the shipped vector |
| --- | --- | --- |
| OL | three quarters | +0.011 (0.003 to 0.021); the whole way +0.010 (-0.001 to 0.021) |
| LB, off the ball | all | +0.001 (-0.002 to 0.005) |
| S | five eighths | +0.002 (0.001 to 0.004) |
| WR | all | +0.009 (0.004 to 0.016) |
| DL | seven eighths | -0.006 (-0.010 to -0.002), and worse at every step, on every set of games |
| LB, on the edge | all | -0.007 (-0.011 to -0.004), the same |

**What moved.** The offensive line three quarters of the way (pass blocking
.60, run blocking .24, awareness .16), where both checks agree and the fit's
interval clears zero. The off-ball linebacker the whole way (coverage .28,
tackling .23, run defence .16, awareness .16, rush .10, speed .07), which the
record allows at every step and the fit does not object to. The safety five
eighths (coverage .36, tackling .17, ball skills .16, speed .14, awareness .09,
run defence .08), as far as the record lets his shortfall grow by a point. And
the receiver the whole way (catching .38, speed .33, routes .20, after the catch
.09), which he measured last time too, on both lifts, and was held on the noisy
check alone. *The line went the rest of the way once the pocket read the lines
(next section).*

The line's move put one man under his record: Jim Ringo '61, a Hall of Fame
centre, from 82 to 81 against a floor of 84. His two blocking ratings sit
within two points of each other, so no weighting does it; he was rated 80 and
82 where every centre of his time in the pool reads 84 to 92 (Bulldog Turner
86, Jim Langer 89, Jim Otto and Mick Tingelhoff 90). The record's own rule is
that a violation a five-point nudge would fix is a rating to correct, not a
disagreement to record, so he is 84 and 86 now, and 85 on either weighting.
The record still reads two violations, Csonka and Riggins.

**What did not.** The defensive line and the edge rusher stay where they were.
On the field a lineman's pass rush is worth four and a half times his run
defence, and moved one attribute at a time in a side of 85s it is worth five
(0.044 against 0.008 a point, across 60 to 96). But generated linemen and edge
rushers rank better on more run defence, not less, on every one of the five
sets of games, and least squares on their averaged production puts a lineman's
run defence level with his rush. The two readings disagree about the defence's
front and neither is taken; why they disagree was not found. The tight end's
free eighth measures nothing (+0.0007) and is not taken, the back is as he was
last time, and the quarterback, the corner and the kickers are not reached by
the lines.

**What it does to ratings.** Nobody moves more than three points. Pass
protectors rise (Andrew Whitworth 89 to 91, David Bakhtiari 84 to 86) and road
graders fall (Larry Allen 96 to 94, Quenton Nelson 94 to 92). Run-stopping
middle linebackers fall two or three (Dick Butkus 92 to 89, Mike Singletary 90
to 87). Ball-hawking safeties rise (Ed Reed 90 to 92, Paul Krause 85 to 87) and
box safeties fall a point or two. Receivers with hands rise two (Larry
Fitzgerald, Calvin Johnson, Cris Carter) and the after-the-catch men fall one.

**What moved with it.** Every club drafts and bids on the table and on
`overall`, so everything read from a drafted league was re-drawn:

| | before | after |
| --- | --- | --- |
| value board: underpaid / about right / overpaid / badly overpaid | OL, CB / S, TE, LB / DL, QB / RB, WR | CB / S, QB, TE, OL / LB, RB, DL / WR |
| value shopper / trenches first / market follower, wins of 14, 24 leagues | 7.7 / 7.5 / 6.2 | 7.8 / 6.8 / 5.2 |
| roster spread, auction / draft | 40.2 / 22.0 | 33.0 / 21.1 |
| a roster at market against the cap, founding | 113% | 117% |
| a quarterback / back at 87, a lineman at 87, a kicker at 88, of a 200 cap | 21 / 10, 9, 5 | 27 / 13, 5, 6 |
| AI offers a league-season / helping the human | 7.3 / 35% | 9.8 / 29% |
| MVPs in 40 seasons, as awarded / read straight | QB 33, RB 7 / QB 37, DL 2, CB 1 | QB 35, RB 5 / QB 39, CB 1 |
| the pass/run read at 0.70 against 0.55: 8, 10, 12 clubs, pro | +0.71, +0.54, +0.56, +0.70 | +0.70, +0.60, +0.40, +0.52 |
| fourth-down aggression four-fifths up, eight clubs / pro | +0.16 / +0.33 | +0.24 / +0.36 |
| blitzing least / the deepest shell, eight clubs / pro | +0.41 / +0.50, +0.36 / nothing | +0.25 / +0.37, +0.41 / +0.27 |
| starter-weeks lost to injury, fantasy / pro, normal | 4.4 / 5.4 | 4.5 / 5.7 |
| a pro league's adjusted net a dropback / yards a carry | 6.13 / 4.58 | 5.97 / 4.59 |
| passers / receivers' targets / backs' carries against real seasons | 1.15 / 0.89 / 1.03 | 1.05 / 0.82 / 0.86 |
| the corner's yardstick, pool / generated | 0.975 / 0.968 | 0.951 / 0.961 |

The trenches-first buyer falls from second to a tie for second with stars and
scrubs, which is the offensive line no longer being a bargain, and the value
shopper still wins most. The corner's yardstick is the engine rather than the
weights, which did not change: the commit that held the lines took it from
0.975 to 0.951 with the old table and weights, the corner's own range
narrowing from 5.6 points to 4.7, most of it in what his own side scores. The
rest is inside the noise a new set of drafts carries; the read and the dials
moved about a standard error either way, and the team page's notes say the
new numbers. The expected-points fit reads 0.0539 a yard against
`EP_PER_YARD`'s 0.0525, inside the audit's band, and is not refitted here.

The injury table's left-hand columns were found stale on the way (see
"Injuries and depth"): they are measured over a thousand games now and read
1.34 injuries a game at the default, not 1.15.

Three tests failed on this and none was a regression in the game. The future
picks test needed a user club that was not already too short to trade, and
seed 9's went into its second draft four men short once the keepers and free
agents came out differently. It could never have failed on seed 9 before
either: that club had a pick to spare, so counting the two future picks it
trades against the limit on finishing short would still have left it under.
It searches seeds now for a club within two men of the limit and not over it,
and a copy of the rule that counts future picks fails it. The free-agent board test
looked for the best-rated free agent in a 400-row window, and he is a punter
ranked 493rd by what he adds, as the test's own comment predicts; it reads the
whole board. The development-pace test held one class's slow developers to a
season behind the fast ones, and that class read 1.09 before and 0.87 after,
with a dozen men in its slow group; five classes read 1.38 and 1.40, and it
pools them now.
...
against a real 5.89; on the drafts of the table and the weights set on it
(below), 5.93, which is the tenth or so two leagues carry either way. Pressure has read the same margin all along, which is
likely why such a league is sacked 7.3% of the time against a real 7.0.

**The value table, a sixth time.** Six rosters at 10,000 games each. In raw
margin per starter a lineman rose from 0.48 to 0.62 (7.3 standard errors) and a
defensive lineman from 0.68 to 0.88 (13.7), and nothing else moved by more than
0.7 of its standard error. The table keeps its starter-weighted total, 86.47,
pinned now rather than re-summed from a rounded table, which had drifted it a
few hundredths a cycle; so every other position gave back about a tenth:

| | QB | CB | RB | TE | DL | WR | S | LB | P | OL | K |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| before | 11.16 | 5.32 | 5.37 | 4.28 | 3.16 | 3.55 | 3.48 | 2.90 | 2.84 | 2.21 | 2.23 |
| now | 10.14 | 4.96 | 4.90 | 3.97 | 3.74 | 3.25 | 3.17 | 2.77 | 2.73 | 2.65 | 2.21 |

The quarterback is 10.14 ± 0.57 between rosters. The offensive line is above
the kicker again. A real lineman's absence still costs his side more than the
engine's eight points worse does, 0.81 points a game against 0.54, and it is
inside the real interval now where it was at the edge of it.

**The rating weights on this engine.** `npm run attrs` at 40,000 games a
reading on every position the pocket reaches:

| position | measured / shipped |
| --- | --- |
| OL | pass blocking .685 / .60, awareness .184 / .16, run blocking .131 / .24 |
| TE | blocking .394 / .25, catching .305 / .25, routes .156 / .20, speed .100 / .10, after the catch .045 / .20 (1.4σ) |
| DL | pass rush .711 / .60, awareness .119 / .08, run defence .118 / .21, tackling .052 / .11 |
| LB, on the edge | pass rush .633 / .45, tackling .149 / .17, run defence .107 / .23, speed .067 / .06, awareness .044 / .09 |
| LB, off the ball | within .03 of the shipped vector on every attribute |
| S | within .03 of the shipped vector, save awareness .144 / .09 and tackling .125 / .17 |

Generated players' production, five sets of 600 games a man, against the move:

| position | half way | all the way |
| --- | --- | --- |
| OL | +0.0001 | -0.0010 (-0.0032 to +0.0012) |
| DL | -0.0022 | -0.0062 (-0.0116 to -0.0016) |
| LB, on the edge | -0.0077 | -0.0187 (-0.0286 to -0.0097) |
| TE | +0.0061 | +0.0087 (+0.0029 to +0.0158) |

**What moved.** The line, the whole way: pass blocking .69, awareness .18, run
blocking .13. The record allows it and generated players neither ask for it nor
object to it. No lineman moves more than two points: pass protectors up (David
Bakhtiari '20 86 to 87, Terron Armstead and Ronnie Stanley 84 to 85), road
graders down (Leonard Davis '07 70 to 69).

**What did not.** The defensive line and the edge rusher measured more pass
rush again, and generated players again rank better on more run defence at
every step (the defensive line on four sets of games in five, the fifth reading
nothing; the edge rusher on all five), so the two authorities still disagree
about the defence's front and neither is taken. The tight end is new: the field
puts his blocking at four tenths of him, generated players agree, and the
record will not have it, because an eighth of the way puts Brock Bowers '24
three points under his floor and the floors for tight ends are receivers'
floors. That is the rule's other exception, decisive about this engine and
contentious about football, so he stays, and the engine's opinion of a blocking
tight end is recorded rather than priced. The safety and the off-ball backer
measured within their noise of where they were set.
...
| | before | after |
| --- | --- | --- |
| value board: underpaid / about right / overpaid / badly overpaid | CB / S, QB, TE, OL / LB, RB, DL / WR | CB / OL, S, QB, TE / DL, LB, RB / WR |
| value shopper / trenches first / market follower, wins of 14, 24 leagues | 7.8 / 6.8 / 5.2 | 7.8 / 7.3 / 5.7 |
| roster spread, auction / draft | 33.0 / 21.1 | 35.6 / 20.4 |
| a roster at market against the cap, founding | 117% | 115% |
| a quarterback / back at 87, a lineman at 87, a kicker at 88, of a 200 cap | 27 / 13, 5, 6 | 25 / 12, 6, 6 |
| AI offers a league-season / helping the human | 9.8 / 29% | 10.4 / 37% |
| MVPs in 40 seasons, as awarded / read straight | QB 35, RB 5 / QB 39, CB 1 | QB 33, RB 7 / QB 38, CB 2 |
| the pass/run read at 0.70 against 0.55: 8, 10, 12 clubs, pro | +0.70, +0.60, +0.40, +0.52 | +0.71, +0.65, +0.66, +0.58 |
| fourth-down aggression four-fifths up, eight clubs / pro | +0.24 / +0.36 | +0.36 / +0.22 |
| blitzing least / the deepest shell, eight clubs / pro | +0.25 / +0.37, +0.41 / +0.27 | +0.25 / +0.42, +0.33 / +0.08 |
| starter-weeks lost to injury, fantasy / pro, normal | 4.5 / 5.7 | 5.3 / 5.2 |
| a pro league's adjusted net a dropback / yards a carry | 5.97 / 4.59 | 5.93 / 4.60 |
| passers / receivers' targets / backs' carries against real seasons | 1.05 / 0.82 / 0.86 | 1.04 / 0.83 / 0.96 |
| generated corners' yardstick | 0.961 | 0.944 |

The trenches-first buyer comes back to second with the lines' worth. The rest
is a new set of drafts: the read and the dials moved about a standard error
either way. The team page's note on the dials says the new numbers, and no
longer names the weak offence with a poor kicker as the squad that gains
nothing, since the squad that gains most now changes from one set of leagues to
the next. The expected-points fit reads 0.0533 a yard against `EP_PER_YARD`'s
0.0525, inside the audit's band, and is not refitted.
...
**The value table, a seventh time.** Six rosters at 10,000 games each. In raw
margin per starter a corner rose from 1.16 to 1.41 (6.4 standard errors), an
off-ball backer from 0.65 to 0.81 (4.4) and a receiver from 0.76 to 0.94 (7.5),
and nothing else moved by more than 0.6 of its standard error. With the
starter-weighted total pinned at 86.47, every other position gave back about
7%:

| | QB | CB | RB | WR | TE | DL | LB | S | P | OL | K |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| before | 10.14 | 4.96 | 4.90 | 3.25 | 3.97 | 3.74 | 2.77 | 3.17 | 2.73 | 2.65 | 2.21 |
| now | 9.39 | 5.60 | 4.67 | 3.75 | 3.71 | 3.46 | 3.24 | 2.88 | 2.60 | 2.44 | 2.12 |

The quarterback is 9.39 ± 0.50 between rosters, 1.68 corners. The value test
held him at 1.8 times any other player; real absences put that ratio anywhere
from about 1.1 to 5, so it now holds the claim at 1.5.

The receiver's rise is the receivers' side of the term, all of it: a receiver
eight points worse lets the coverage hold the ball, and his side is sacked 0.20
points more, where his real absences say +0.03 (-0.18 to +0.28). That is inside
the interval and a long way from its middle, and about a fifth of a receiver's
worth in the table now rests on it. The first receiver alone is worth 1.20
against the position's 0.94, about a quarter more (1.01 against 0.76 on the
table before).

**The rating weights on this engine.** `npm run attrs` at 40,000 games a
reading on the positions the term reaches:

| position | measured / shipped |
| --- | --- |
| CB | coverage .547 / .51, ball skills .147 / .19, speed .139 / .16, awareness .126 / .09, tackling .041 / .05 |
| WR | routes .362 / .20, catching .311 / .38, speed .262 / .33, after the catch .065 / .09 |
| LB, off the ball | coverage .380 / .28, awareness .192 / .16, tackling .163 / .23, run defence .131 / .16, pass rush .075 / .10, speed .060 / .07 |
| LB, on the edge | pass rush .599 / .45, run defence .090 / .23, the rest within .03 |

Generated players' production, five sets of 600 games a man, against the move:

| position | half way | all the way |
| --- | --- | --- |
| CB | +0.0003 | +0.0005 (+0.0001 to +0.0010) |
| WR | +0.0040 | +0.0055 (-0.0008 to +0.0122) |
| LB, off the ball | +0.0003 | +0.0000 (-0.0015 to +0.0016) |

**What moved.** The corner and the receiver the whole way: coverage .54, ball
skills .15, speed .14, awareness .12, tackling .05; routes .37, catching .31,
speed .26, after the catch .06. The record allows both and generated players
ask for both. The off-ball backer five eighths: coverage .35, tackling .19,
awareness .18, run defence .14, pass rush .08, speed .06, as far as the record
lets his shortfall grow by a point; generated players neither ask for it nor
object to it. Route running is the receiver's largest weight now, and all of
that is the receivers' half of the term above: route runners up two (Davante
Adams '20 91 to 93, Reggie Wayne '07, Michael Irvin '91, Steve Largent '85),
returners playing receiver down three or four (Devin Hester '07 64 to 61,
Cordarrelle Patterson '13 62 to 58). No corner and no backer moves more than a
point: coverage backers up (Jack Ham '75 92 to 93, Derrick Brooks '02 90 to
91), thumpers down (Bill George '56 89 to 88). The record still reads Csonka
and Riggins.

**What did not.** The edge rusher measured more pass rush again, .599 against
.45, which generated players have objected to at every step before; the term
leaves him out of coverage, so he holds. The safety, the defensive line and the
tight end were not re-measured: their worth in the table did not move.
...
| | before | after |
| --- | --- | --- |
| value board: underpaid / about right / overpaid / badly overpaid | CB / OL, S, QB, TE / DL, LB, RB / WR | CB / OL, S, LB, QB / TE, DL / RB, WR |
| the corner's value / the quarterback's | 1.63× / 1.01× | 1.86× / 0.95× |
| value shopper / trenches first / market follower / spread it evenly, wins of 14, 24 leagues | 7.8 / 7.3 / 5.7 / 5.3 | 7.6 / 6.8 / 4.5 / 4.2 |
| roster spread, auction / draft | 35.6 / 20.4 | 36.9 / 21.1 |
| a roster at market against the cap, founding | 115% | 115% |
| a quarterback / corner / back at 87, a lineman at 87, a kicker at 88, of a 200 cap | 25 / 12 / 12, 6, 6 | 23 / 14 / 11, 6, 5 |
| AI offers a league-season / helping the human | 10.4 / 37% | 9.7 / 40% |
| MVPs in 40 seasons, as awarded / read straight | QB 33, RB 7 / QB 38, CB 2 | QB 33, RB 7 / QB 26, CB 14 |
| the pass/run read at 0.70 against 0.55: 8, 10, 12 clubs, pro | +0.71, +0.65, +0.66, +0.58 | +0.31, +0.57, +0.71, +0.40 |
| fourth-down aggression four-fifths up, eight clubs / pro | +0.36 / +0.22 | +0.02 / +0.25 |
| blitzing least / the deepest shell, eight clubs / pro | +0.25 / +0.42, +0.33 / +0.08 | −0.07 / +0.33, +0.24 / +0.06 |
| starter-weeks lost to injury, fantasy / pro, normal | 5.3 / 5.2 | 4.7 / 5.4 |
| a pro league's adjusted net a dropback / yards a carry | 5.93 / 4.60 | 5.93 / 4.61 |
| passers / receivers' targets / backs' carries against real seasons | 1.04 / 0.83 / 0.96 | 1.02 / 0.82 / 1.02 |
| the corner's yardstick / generated corners' | 0.954 / 0.944 | 0.957 / 0.963 |

The read's eight-club figure is the mean of four sets of drafts and the pro
one of three; the first set read +0.12 and +0.19, and on the drafts before the
table and the weights moved the coverage alone read +0.87 in the same
eight-club leagues. So the gain from throwing is about half what it was in
eight-club leagues, most likely through who the clubs draft; throwing is
still the best setting on every draw. The two buyers who buy what the room buys
lost about a win each, about two standard errors of a difference each, most
likely because the corner the room underpays is worth more; neither is
isolated. The eight-club dials are one set of drafts, the one whose read was
lowest of the four, and aggression there has read anywhere from +0.02 to +0.39
over five; the team page's note says up to a quarter of a point and that it
moves. The MVP discount on every position but the quarterback and the back went
from 0.6 to 0.45 (see "Awards, records and the hall of fame"), which keeps the awards at 33 and 7 and the
nearest corner at 73% of a winner. The expected-points fit reads 0.0518 a yard
against `EP_PER_YARD`'s 0.0525, inside the audit's band, and is not refitted.
Of the lines' checks, the lineman's passing effect reads 0.165 at the audit's
2,000 games and the defensive lineman's 0.145, against 0.130 and 0.164 when the
pocket went in; both inside their bands, and at even sides the coverage term
is one, so most likely that is the random stream the term re-drew rather than
the term.
...
Measured again once the drive was held to real play-by-play, with the quarterback at 8.68 in the table: read straight it names 37 quarterbacks, 2 defensive linemen and a corner in the same forty seasons, and with both numbers as they are, 33 quarterbacks and 7 backs again, with no other position within 63% of a winner. Neither number moved. And once the lines were held to real absences, with the quarterback at 11.16 and a sack credited the way the real game spreads them: read straight, 39 quarterbacks and a corner, the two linemen gone with the credit that had given the best rusher two thirds of his side's sacks; as awarded, 35 quarterbacks and 5 backs, nobody else within 57%. Real voting's four in five and one in five is still the target, and neither number moved. And once the pocket read the lines, with the quarterback at 10.14: read straight, 38 quarterbacks and 2 corners; as awarded, 33 and 7 again, nobody else within 69%. Neither number moved.

Once the coverage made sacks, with the corner at 5.60 against the quarterback's 9.39, the table read straight names 26 quarterbacks and 14 corners. As awarded the forty seasons still name 33 quarterbacks and 7 backs, but with a corner's season 97% of a winner's, and every earlier table had kept the nearest other position at 57% to 77%. So `MVP_OTHERS` moved for the first time:

| every other position at | QB | RB | the best other season, of a winner's |
| --- | --- | --- | --- |
| 0.6 | 33 | 7 | 97% |
| 0.55 | 33 | 7 | 89% |
| 0.5 | 33 | 7 | 81% |
...
It is 0.45: the same 33 and 7, and the old margin back. `MVP_QB_OVER_RB` did not move.
...
Aggression helps every mix of offence and kicker in both (+0.50 to +0.80 in eight-club leagues, +0.60 to +1.05 in pro ones). Blitzing least reads a little lower in pro leagues than it did (+0.23 from +0.34, about one standard error) and the deepest shell lower in eight-club ones (+0.38 from +0.66); why is not established. The team page's notes say the new numbers.

**And again once the drive was held to real play-by-play** (see "The drive model, held to real play-by-play"):

| dial, against its default | eight-club leagues, 6,400 club-games | pro leagues, 12,800 |
| --- | --- | --- |
| 4th-down aggression at 0.8 | +0.39 ± 0.14 | +0.31 ± 0.10 |
| 4th-down aggression at 1.0 | +0.37 ± 0.18 | +0.14 ± 0.13 |
| Blitz at 0.05 | +0.41 ± 0.20 | +0.50 ± 0.14 |
| Deep coverage at its best | +0.36 ± 0.23 at 0.6 | nothing (+0.01 ± 0.15 at 0) |
| Tempo at any setting | within ±0.22 | within ±0.24 |

Aggression is worth half what it was in eight-club leagues and about a third in pro ones, and its best setting is no longer the top of the dial. Most likely that is the fourth-down call itself: the default is now fitted to real coaches' decisions, which go for it more than the old margin of 1.15 points did, so there is less left for the dial to find. That was not isolated. A weak offence with a poor kicker gains nothing measurable at any setting (+0.17 ± 0.21 and +0.16 ± 0.19 at 0.8, −0.08 at the top in both). Blitzing least reads higher in pro leagues (+0.50 from +0.23) and the deepest shell is flat in them (from +0.15 ± 0.15). Tempo is flat for every club taken together. One third of eight-club leagues, the middle one, read +1.26 ± 0.42 at the fastest setting; on eight fresh leagues the same third read −0.66 ± 0.44 there, so it was noise, and the sixteen leagues together keep every setting within about a quarter of a point of the default. The team page's notes say the new numbers, and no longer say that every dial leans one way for every kind of squad. The audit replayed aggression on four of the eight-club leagues, 100 games a club: +0.48 four-fifths of the way up the dial. Once the rating weights were re-measured on the drive model (see "The rating weights, re-measured on the engine as it now is") the drafts came out differently, and on the new drafts four-fifths of the way up reads +0.16 ± 0.14 in the eight eight-club leagues and +0.33 ± 0.10 in the pro ones, where it read +0.39 and +0.31; the four the audit replayed read −0.03. A difference between two sets of drafts that size is about one standard error, so the dial pays a sixth to a third of a point four-fifths of the way up, and going for it less costs a fifth to a quarter (−0.21 ± 0.09, −0.28 ± 0.07 at the bottom). Once the lines were held to real absences and the table and the weights re-set on them, four-fifths of the way up reads +0.24 ± 0.14 and +0.36 ± 0.10, and going for it less costs a sixth (−0.17 ± 0.10, −0.17 ± 0.07); a weak offence with a poor kicker still gains nothing measurable (+0.25 ± 0.27, +0.12 ± 0.17 at 0.8). Blitzing least reads +0.25 and +0.37, the deepest shell +0.41 and +0.27, and tempo stays within a quarter of a point. Once the pocket read the lines and the table and the line's weights were re-set on it, four-fifths of the way up reads +0.36 ± 0.14 and +0.22 ± 0.10, and going for it less costs a quarter to a third (−0.31 ± 0.10, −0.23 ± 0.07). A weak offence with a poor kicker reads +0.10 ± 0.27 and +0.46 ± 0.17 at 0.8, where it had read next to nothing on every set of drafts before; the kind of squad that gains most is a different one in the two kinds of league on the same drafts, so it is noise in the split, and the team page no longer names one. Blitzing least reads +0.25 and +0.42, the deepest shell +0.33 and +0.08, and tempo stays within a quarter of a point. Once the coverage made sacks and the table and the weights were re-set on it, four-fifths of the way up reads +0.02 ± 0.14 and +0.25 ± 0.10, and going for it less costs a third (−0.36 ± 0.10, −0.34 ± 0.07). Over the five sets of eight-club drafts it has been measured on, four-fifths of the way up has read +0.39, +0.16, +0.24, +0.36 and now +0.02, so the team page says up to a quarter of a point and that it moves from one set of leagues to the next. Blitzing least reads −0.07 ± 0.20 and +0.33 ± 0.14, with 0.15 the best eight-club setting at +0.18 ± 0.16, and blitzing more costs a quarter to a half; the deepest shell reads +0.24 and +0.06, and tempo stays within a fifth of a point. The audit now replays the pro leagues, whose error is half the eight-club ones': +0.25 at 0.8, flagged if it leaves −0.05 to +0.55.
...
The last column was measured while that read was still in place, so the human's club was playing it as everybody's opponent too; the others with the new one. *Measured again the same day once the passer and the receivers were held to real seasons (see "The passing game, held to real quarterbacks"): +0.68 ± 0.17 in eight-club leagues, +1.00 in ten, +0.81 in twelve and +1.04 in pro ones; every fifth of every league still does best at 0.65 or 0.70, gaining +0.13 to +1.68 at 0.70. The most run-built fifth of eight-club leagues is the nearest to flat, +0.13 ± 0.38. Always 0.35 costs −1.04 to −1.23. And again once the drive was held to real play-by-play (see "The drive model, held to real play-by-play"): +0.71 ± 0.18 in eight-club leagues, +0.54 in ten, +0.56 in twelve and +0.70 in pro ones. Eighteen of the twenty fifths do best at 0.65 or 0.70, gaining −0.02 to +1.52 at 0.70, and the two that do not are within their noise of it; always 0.35 costs −0.23 to −0.93. And again once the lines were held to real absences (see "The trenches, held to real absences"): +0.70 ± 0.18 in eight-club leagues, +0.60 in ten, +0.40 in twelve and +0.52 in pro ones. Eighteen of the twenty fifths do best at 0.65 or 0.70; the two that do not, one at 0.55 and one at 0.45, are within their noise of it, ± 0.4 a fifth. The team page now says four-tenths to seven-tenths of a point. And again once the pocket read the lines (see "The pocket, held to real absences"): +0.71 ± 0.18 in eight-club leagues, +0.65 in ten, +0.66 in twelve and +0.58 in pro ones. Nineteen of the twenty fifths do best at 0.65 or 0.70, and the one that does not, a pro league's most run-built fifth, peaks at 0.45 within its noise. The team page's words stand. And again once the coverage made sacks (see "Coverage sacks, held to real absences"): +0.12 ± 0.18 in eight-club leagues on the first set of drafts, so three more were drawn, +0.30, +0.46 and +0.36, a mean of +0.31; +0.57 in ten and +0.71 in twelve; and in pro ones +0.19, +0.65 and +0.35 on three, a mean of +0.40. The gain in eight-club leagues is about half what it was. On the drafts before the table and the weights moved, the coverage alone read +0.87 in the same leagues, so it is the table and the weights, most likely through who the clubs draft, rather than the coverage; why is not isolated. Throwing is still the best setting in every kind of league on every draw, and always 0.35 costs −0.24 to −0.73. Eighteen of the twenty fifths on the first draws do best at 0.65 or 0.70; the two that do not, a ten-club league's second most pass-built fifth at 0.55 and again a pro league's most run-built at 0.45, are within their noise of it. The team page now says three-tenths to seven-tenths of a point.* The ramp fitted to these games puts the crossover at the right-hand end of the range it is searched over, and in-sample it beats always throwing by five hundredths at most. So there is no crossover: `CROSSOVER` is `null` in both kinds of league, and the read tells every squad to throw — a run-built squad in so many words, "Even so, squads built like yours win more by throwing, measured against the clubs you play" — and says moving the dial is worth three-tenths to seven-tenths of a point a game. The ramp stays in the code for the day a crossover comes back. The read of 19 September, fitted on synthetic rosters before the run game was broken, had the direction right all along.

A dial that goes one way for every squad is a tax on anybody who does not max it. It is not in itself unrealistic: a real dropback was worth +0.007 expected points in 2022–23 and a designed run −0.062, and a dropback was worth more in 46 of 64 team-seasons. This section used to say that what keeps real teams running is defences that adjust to how often an offence throws, which the engine's never do. That was never measured, and measured it does not hold. On early downs in neutral games from 2016 to 2024 (first and second down, win probability 20% to 80%, outside the last two minutes of a half, between the 20s), a dropback was worth +0.101 expected points and a designed run −0.087: a gap of 0.19, between 0.14 and 0.22 in every season, while teams threw 53% of the time. Real defences do adjust. The same defence, against an offence that throws ten points more than expected in its other games, puts 0.11 fewer men in the box (0.10 to 0.13) and 0.09 more defensive backs on the field (0.07 to 0.10), with the offence's personnel, formation and the situation held (98,825 plays with participation data, resampling offence-seasons). But a man fewer in the box is worth +0.032 a designed run and −0.018 a dropback, and a defensive back more +0.028 and −0.024, with the offence held to its own season. So ten points more throwing costs its dropbacks about 0.004 and gives its runs about 0.006: a fiftieth of the gap. Only the box and the personnel are counted here; shells, blitzes and in-game adjustments are not. The one-ended dial is the real game's incentive, then, not the engine's mistake: whatever keeps real teams running, expected points cannot see it. A defence that adjusted as real ones do would not move the read, so it is not built. Making the dial a real choice would take a price the real game does not charge, which is a design decision rather than a measurement. The passing game's own heat, the other thing recorded here, is mostly gone: drafted leagues throw for 7.8 yards an attempt against a real 7.1, from 8.1. The audit re-measures it on eight of those leagues, 150 games a club at each setting: +0.12 against 0.55 once the coverage made sacks (+0.71 before), the lowest of the four sets of drafts above, flagged outside −0.28 to +0.52.

**The AI clubs were fine, give or take the opening weeks** — on the engine before the run game was fixed, and not measured since. Every club in four pro leagues played its own league with its pass rate moved, paired by seed (`npm run passrate -- ai`). Moving every club the same way, anywhere from −0.20 to +0.10, was worth −0.13 to +0.29 — flat — and flat again at midseason. The weather finding did not replicate. What is left is fit: moved each to its own read at kickoff, AI clubs would gain +0.46 ± 0.14 a game, nearly all of it run-built clubs that open too pass-heavy (Ground & Pound +0.96, the most run-built third +1.02), and the weekly drift walks those down by midseason — Ground & Pound from 0.44 to 0.33, after which running still more earns nothing. Starting AI clubs on the read is a possible change, not made. Measured again on the engine as it now is, with the table set once coverage made sacks (`npm run passrate -- ai 4 100 kickoff 41`): moved to the read at kickoff, every AI club gains +0.22 ± 0.15 a game, the most run-built third +0.55 ± 0.26, Ground & Pound +0.09 ± 0.43 and Air Raid +0.02; their game plans and the season's drift already take some of what the read offers. It was proposed as what a harder difficulty setting could mean, smarter coaches rather than better players, and it is not built: a quarter of a point a game is about a tenth of a win a season for each club, less still for the human playing them, and every personality would throw at 0.70.
...
**Saves are packed, because thinning was not enough.** Measured again a year of features later, the wall had come back. A pro league — 32 clubs, drafted, seed 5 — as the store writes it, in characters:

| | at kickoff | end of season, logs not yet thinned |
| --- | --- | --- |
| season 1 | 246,000 | 2,407,000 |
| season 2 | 508,000 | 2,675,000 |
| season 8 | 1,015,000 | 3,160,000 |

Chromium 141 gives an origin 5.24 million characters of localStorage (filled until it refused: 5,242,546), so two pro leagues both at the end of their second season no longer fitted, and the third slot was a promise the browser could not keep. Nor is the growth the 30 KB a season the box-score note near the top of this document measured. Kickoff grows about 85,000 characters a season, and 37,000 of that is `league.transactions`, a log nothing ever trims; the rest is mostly rookies (16,000, levelling off), careers (12,000), `dev` (7,000, levelling off) and the archive of past seasons (7,000).

`savecodec.js` deflates the JSON — fflate 0.8.2, vendored unmodified in `src/vendor/`, since the app has no build step — and packs the bytes fifteen bits to a character, offset past the control characters, so every character is a code unit from 0x20 to 0x801F: never half of a surrogate pair, which a browser is free to replace. Base64 would have been a third larger again. A packed character costs the origin what an ASCII one does — filled with either, Chromium holds the same 5,242,546 — and the same league packs to a tenth or a twelfth:

| | JSON | level 1 | level 6 |
| --- | --- | --- | --- |
| pro, end of season 1 | 2,407,000 | 230,000 | 189,000 |
| pro, end of season 8 | 3,160,000 | 314,000 | 257,000 |
| 12-club fantasy, end of season 8 | 1,699,000 | 173,000 | 141,000 |

Three pro leagues at the end of their eighth season, packed at the larger level, are 942,000 characters: 18% of the origin.

**Where the time goes.** Packing costs time, and a watched game saves after nearly every play. Measured on a pro save at its peak (2.64 million characters), with Chromium's CPU slowed four times to stand in for a mid-range phone:

| | ms |
| --- | --- |
| serialize it (`JSON.stringify`) | 136 |
| write it as JSON | 152 |
| deflate it, level 6 / level 1 | 642 / 373 |
| pack the bytes into characters | 65 |
| write it packed | 4 |
| read it back: unpack and inflate | 193 |
| the browser's own `CompressionStream`, for comparison | 743 |

So routine saves are packed in a worker (`save-worker.js`). The page serializes and writes — 140 ms, where an unpacked save cost it 288 — and the 707 ms of packing happens beside it. Where a save has to land before anything else happens — the page hidden or closed, a slot switched, emptied or opened, an import, the first write into a new slot — it is packed on the spot at level 1: 578 ms against the 288 an unpacked save took, which is the price of being certain, paid only where certainty is the point. Loading the largest save costs 193 ms more than it did. `CompressionStream` was measured and passed over: slower here, asynchronous only, so of no use to a page that is going away, and with no level to choose. IndexedDB was the other road, and is asynchronous too; packing keeps localStorage's synchronous write, which the flushes depend on, and buys the room.

**The order saves land in.** The worker is given one save at a time. A change made while one is being packed waits for it and goes as soon as it lands, from the state as it is by then: posting every change would let a fast autoplay on a slow phone outrun the packing, with a copy of the league queued for every play. A save made on the spot supersedes whatever the worker holds — each save the worker is given carries a number, and an answer that is not the one outstanding is dropped, so an older league never lands over a newer one. Deleting the open league drops what the worker holds for it, as it already cancelled the debounced write (the first race below). A packed save the browser refuses is reported as a refused write on the page is, and retried on the next change rather than at once, or a full origin would have the worker packing the same league in a loop. A browser that cannot run a module worker fails it when it is made or through its `error` event; either way every save from then on packs on the spot, and the one the worker was holding is done again then. `tests/savecodec.test.js` holds each of these rules against a fake worker the test answers by hand, and each test fails if its rule is taken out. `scripts/smoke.mjs` reads saves through the app's own codec, checks in a real browser that a routine save was packed by the worker — deflate is deterministic, so packing what is stored again at level 6 gives it back exactly — and that a change made just before a reload survives it. Its first run under the worker failed one check for a reason worth keeping: the smoke writes saves into localStorage under the running app and reloads to load them, and a routine save still in the worker was flushed over the injected one on the way out — the app doing exactly what it should, into a test that assumed the debounce was the only thing in flight. Every injection now fires the app's own pagehide flush first.

**Old saves, and going back.** A save written before packing is plain JSON and loads as it always did — `decode` passes through anything without the marker — and the next save packs it. An exported file is still plain JSON and imports as before. Going *back* is the danger. The code before this reads a packed slot as unreadable, `read()` turns unreadable into empty, and the next save writes the empty league over the packed one. Code already shipped cannot be changed, so undoing this is a new release that keeps `decode`, never a revert.

This is headroom, again, rather than the ceiling removed — so the quota error in `store.js` still has to work, and the size shown against each slot on the home screen, which is characters stored, now reads a tenth of what it did against the same 5.24 million. How much headroom is an extrapolation, not a measurement: at the rate measured, packed saves grow about 12,000 characters a season at their peak, and three slots would take more than a hundred seasons to fill the origin. `league.transactions` grows fastest, and faster each season — 32,000 characters a season over seasons two to five, 41,000 over five to eight — and would be the thing to trim if that ever mattered.
...
correct weight was 0.00. *Every position was measured again once the drive was held to real
play-by-play, and moved by the same rule with one tightening: see "The rating
weights, re-measured on the engine as it now is".*
...
measured against real carries, 3.90 against 9.02 once the passer, the
receivers and the backs' other channels were too, and is 4.17 against 8.68
since the drive around them was held to real play-by-play.
...
Vanilla ES modules, hash router, one persisted state object (`store.js`). Views re-render from state; the live game view manages its own DOM and autoplay timer. Everything is relative-path so it deploys to a GitHub Pages subpath. Service worker: network-first for HTML, cache-first for assets (bump `CACHE` in `sw.js` on every release or installed clients keep the old CSS). The precache list is explicit, and three modules shipped without being on it — team statistics, the side-by-side comparison and the contract-length prices. Online that is invisible, since a missing module is fetched and cached on first use; offline, the first launch after an update fails, because activation deletes the old cache and the module was never in the new one. `tests/precache.test.js` now walks the import graph from `main.js` and fails on any module the list does not carry, and on any entry that no longer exists, since `cache.addAll` is all or nothing and one 404 leaves the old app installed for good. **What launches offline** is the one copy of the app's page the worker keeps, and it used to be whatever page was opened last inside its scope: a mistyped address kept its 404, and DESIGN.md — the whole repository is published — kept that, so the next launch with no network showed a blank error page or the design notes instead of the game (both reproduced in Chromium). Only the app's own page, `./` or `./index.html` with any query, answered whole — a success, not redirected, and HTML — may now become that copy; anything else is passed on as the server sent it and kept nowhere. A release clears a copy already spoiled, because installing fetches the page afresh. `tests/sw.test.js` runs the worker in a sandbox with a network and a cache it controls, and two of its four tests fail on the old handler; the smoke opens a 404 and DESIGN.md, stops its server and launches the app.
