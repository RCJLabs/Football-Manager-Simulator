...
**Known, measured, not fixed: a rookie contract does not survive its first offseason.** `keeperLimit` has said since the cap shipped that under a cap the contract decides rather than a quota — four-year rookie deals turning over about a quarter of a roster a year. `aiKeepers` does not implement it: it re-runs the whole roster through a surplus test (`market × taste × BIRD_IN_HAND − cost > 0`) every offseason, so a contract means nothing after the year it was signed in. It falls hardest on the draft, which is priced above market on purpose. Measured: a class drafted 155 strong comes back the next season 17 strong, its own clubs having let nearly all of it go with three years still to run; it then climbs back to about fifty as the same players are re-signed off the street. The aggregate still works — generated players hold 696 of 864 slots by season thirteen — but the churn is not football. The fix is to treat a man with years left as kept rather than chosen, and it was tried: it fails in `validateKeepers`, which rejects a keeper list over the cap, and a club's own committed contracts can legitimately be over it. That is a cap-semantics and keeper-screen change, so it is its own piece of work rather than a rider on this one.
