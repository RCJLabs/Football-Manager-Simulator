...
**Known, measured, not fixed: cutting is free, so the churn moved in-season.** With the keeper round fixed, the dominant loss of a drafted class is no longer the offseason. Traced through one season of one class: of 105 rookies on rosters at kickoff, 51 were gone a year later — **36 of them on in-season waivers** and only 14 released at the keeper round. `aiFileClaims` drops a man under contract for any free agent two points better that adds six to the lineup, and it costs the club nothing to do it, because a cut simply deletes the contract. The real answer is dead money: a club that cuts a man under contract keeps paying him. That is one rule that would make contracts bite in the keeper round, in `cutToCap` and on the wire at once, but it needs a per-club ledger counted in `capHit` and it would change cap pressure everywhere, so it is its own piece of work rather than a rider on this one.
